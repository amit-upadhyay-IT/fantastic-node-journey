...
This is the example of real-time chat communication. But here you will see that if you send "hello", then it will appear to you also (on your screen). So instead of doing an `emit` on server side, we can do a `broadcast`.
...
```
Lets say you want to have a channel `chat message` where you only want to receive on the server side and send from client side. But you may want another channel to send from the server side and receive on the client side.

Lets create another channel `send message` to send from the server side.
...
socket.broadcast.emit('send message', msg);
```
here you can see that the `msg` is being sent via the `send message` channel. Now you need to listen on `send message` on the client side.
...
So this way you can have multiple channels, one channel has two methods: `emit` and `on`. `emit` will send data from source to destination and `on` will send from destination to source in case of the client-server kind of scenarios.
